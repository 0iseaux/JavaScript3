...
function main(url) {
    const fetchJSON = async function fetchJSON(url) {
        const fetchRepoData = await fetch(url);
    ...
    function sortRepo(repoData) {
        let sorted = Object.keys(repoData)
            .sort((a, b) => {
                a = repoData[a].name.toLowerCase();
                b = repoData[b].name.toLowerCase();
                if (a === b) return 0;
                return a < b ? -1 : 1;
            })
            .reduce((acc, curr, i) => {
        ...
        appendRepoData(repoNamesArr)
    }

    // create list of repo names from Arr to select from
    function appendRepoData(repoNamesArr) {
